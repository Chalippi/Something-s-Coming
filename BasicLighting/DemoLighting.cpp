...
void DemoLighting::Update(float deltaTime)
{
...
void DemoLighting::UpdateSprite(float deltaTime)
{
...
void DemoLighting::MoveSprite(float deltaTime)
{
	if ((handling->IsKeyPressed(AXIS_X_LEFT)
		|| handling->IsKeyPressed(SDLK_a))&& cameraPos.x <= 42) {
		xpos += (SPEED / 2);
		xposlight += (SPEED / 2);
	}
	if ((handling->IsKeyPressed(AXIS_X_RIGHT)
		|| handling->IsKeyPressed(SDLK_d)) && cameraPos.x >= -11) {
		xpos -= (SPEED / 2);
		xposlight -= (SPEED / 2);
	}
	if ((handling->IsKeyPressed(AXIS_Y_UP)
		|| handling->IsKeyPressed(SDLK_w)) && false) {
		zpos += (SPEED/2);
		zposlight += (SPEED / 2);
	}
	if ((handling->IsKeyPressed(AXIS_Y_DOWN)
		|| handling->IsKeyPressed(SDLK_s)) && cameraPos.z >= 0 && false) {
		zpos -= (SPEED/2);
		zposlight -= (SPEED / 2);
	}
